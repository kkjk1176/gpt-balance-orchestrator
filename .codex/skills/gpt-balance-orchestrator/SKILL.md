...
Use child-agent sessions for feature units whenever the environment exposes child-agent delegation. Give each session a bounded brief containing the goal, acceptance tests, owned paths, forbidden/adjacent paths, dependencies, validation command, and its assigned route. In shared-workspace environments, never delegate overlapping writes concurrently. If separate user-owned tasks are available only on explicit user request, use child agents rather than creating user-owned tasks without that request.

## Delegation lifecycle

Treat delegation as a parent-owned lifecycle, not a fire-and-forget handoff. Use child-to-parent final reports as the default completion mechanism; use active status tracking only as an exception. The agent that creates a child remains responsible for evaluating the child's final result and must not treat the work as complete before receiving that result or an explained blocker.

- **Main session:** Own the overall plan and every delegated workstream. Maintain lightweight state for each workstream: owner, expected result, dependency or risk level, and the condition that makes a delayed report actionable. Continue integration work while awaiting reports; do not continuously poll healthy children. Do not finalize the user request or report completion while a required final report is absent. Consolidate reported results, resolve dependencies and conflicts, and communicate the integrated outcome.
- **Child session:** Complete its bounded assignment and may delegate non-overlapping subtasks. Require a final report from each delegated child before treating the corresponding subtask as complete, but avoid continuous polling when it is healthy. Incorporate those reports, run assigned validation, and send a concise final report to its parent containing the outcome, changed paths, checks run, unresolved risks, and any blocked work.
- **Child agent:** Complete its assigned scope, run required validation where possible, and proactively report its final outcome to its parent. Do not silently stop after partial progress; explicitly report blockers, failed checks, or remaining work.

Actively check a child's status only when its expected reporting point has passed, it emits a failure or blocker signal, an upstream dependency needs an answer, or the work is high risk. Use bounded status waits rather than unbounded blocking waits. If a child is blocked, fails, or returns an incomplete result, keep ownership at the parent: clarify, retry, re-route, or escalate before closing the parent assignment. Report upward only after the delegated subtree has final reports with evaluated results, or has a clearly explained, user-visible blocker that prevents further progress.
...
Use the following default model and reasoning level. Set an explicit model override only when the delegation API supports it. If the requested model is unavailable, choose the lowest available model that preserves the route's intent and say so in the handoff; in this runtime, map Luna routes to Terra at the stated reasoning level rather than claiming Luna was used.

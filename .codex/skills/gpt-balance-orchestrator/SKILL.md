...
- **Main session:** Own the overall plan and every delegated workstream. Act only as coordinator: direct child sessions, analyze their reports and artifacts, make follow-up and re-routing decisions, resolve dependencies and conflicts, and synthesize the integrated outcome. Do not execute feature work yourself, including file edits, code changes, tests, builds, tool-driven validation, or remediation; delegate it to a child session or one of its child agents. Maintain lightweight state for each workstream: owner, expected result, dependency or risk level, named completion-report channel, recovery action, and the condition that makes a delayed report actionable. Do not continuously poll healthy children. A final report counts only after the main session observes automatic delivery or retrieves it through the named channel. Do not finalize the user request or report completion while a required final report is absent.
...
Assign every unit to a child session. That session, or its child agents, owns this loop; the main session may coordinate and assess reports but must not perform any loop step itself.
...
1. Have a child session integrate completed units in dependency order and resolve interface mismatches with the owner of the affected unit. The main session integrates reports and decisions, not implementation artifacts directly.
2. Have a child session run all focused checks plus the smallest relevant integration, lint, type, or build gate. Add or strengthen regression coverage when a defect or review finding revealed a gap.
